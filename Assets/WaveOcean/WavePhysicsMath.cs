    ...
    public static class BoatPhysicsMath
{
    ...
    public static Vector3 BuoyancyForce(float rho, TriangleData triangleData)
    {
        ...
        //Debug.Log(buoyancyForce + ", trig: " +triangleData);
    ...
    public static Vector3 PressureDragForce(TriangleData triangleData)
    {
        ...
        if (triangleData.cosTheta > 0f)
        {
            ...
            float C_PD1 = WaveDebugPhysics.current.C_PD1;
            float C_PD2 = WaveDebugPhysics.current.C_PD2;
            float f_P = WaveDebugPhysics.current.f_P;
        ...
        else
        {
            ...
            //To change the variables real-time - add the finished values later
            float C_SD1 = WaveDebugPhysics.current.C_SD1;
            float C_SD2 = WaveDebugPhysics.current.C_SD2;
            float f_S = WaveDebugPhysics.current.f_S;
    ...
    public static Vector3 SlammingForce(SlammingForceData slammingData, TriangleData triangleData, float boatArea, float boatMass)
    {
        ...
        //float p = WaveDebugPhysics.current.p;

        //float acc_max = WaveDebugPhysics.current.acc_max;

        float p = 2f;

        float acc_max = acc;

        float slammingCheat = WaveDebugPhysics.current.slammingCheat;
